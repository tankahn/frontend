...
//   const endPoint = 'http://apis.data.go.kr/B552468/acdntFreqocZone'
...
//   )
// }
function fetchData(signguCode) {

  const endPoint = 'https://apis.data.go.kr/B552468/acdntFreqocZone/getAcdntFreqocZone'
  const serviceKey = process.env.REACT_APP_SERVICE_KEY;
  // const siDo = 11;
  // const signguCode = 440;
  const dataType = 'json';
  const numOfRows = 10;
  const pageNo = 1

  const promise = fetch(`${endPoint}?serviceKey=${serviceKey}&pageNo=${pageNo}&numOfRows=${numOfRows}&dataType=${dataType}&signguCode=${signguCode}`)
    .then(res => {
      if (!res.ok) {
        throw res;
      }
...
export default function App() {
  ...
  const [isLoaded, setIsLoaded] = useState(false);
  const [data, setData] = useState(null);
  const [signguCode, setSignguCode] = useState(11);
  ...
  return (
    <div style={{margin: "1rem"}}>
      <h1>{signguCode}지역 이륜차 사고 조회 &#128561;</h1>

      {/* <h2>조회하실 연도를 선택하십시오</h2>
      <div className="">
        <button onClick={() => setSignguCode(searchSignguCode - 1)}>&#10094; 이전년도</button>
        <button onClick={() => setSignguCode(searchSignguCode + 1)}>다음년도 &#10095;</button>
      </div> */}
